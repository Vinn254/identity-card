...
(function () {
  // Toggle: set to false to force real backend
  window.USE_DEMO_MODE = false; // <-- set to true only for static demo purposes
  ...
  window.addEventListener('DOMContentLoaded', () => {
    if (!window.USE_DEMO_MODE) return;
    ...
    window.__demoApi = async function (path, opts = {}) {
      ...
      if (path === '/auth/register' && method === 'POST') {
        ...
        save(STORE_KEYS.session, { token, user: publicUser(u) });
      ...
      if (path === '/auth/me') {
        const s = load(STORE_KEYS.session, null);
        if (!s) throw new Error('Not authenticated');
        return { user: s.user };
      }

      const session = load(STORE_KEYS.session, null);
